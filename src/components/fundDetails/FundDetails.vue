<template>
  <v-container>
        ...
        <v-container>
          ...
          <v-row>
            <v-col style="font-family: PT Sans Caption ; font-weight:bold; padding:1px"
              >Tokens in Circulation: {{ getCollectionDetails.totalSupply || 0 }} {{ getCollectionDetails.symbol }}</v-col
            >
          </v-row>
          <v-row>
            <v-col style="font-family: PT Sans Caption ; font-weight:bold; padding:1px">
              Tokens Owned: {{ getCollectionDetails.userTokenBalance }} {{ getCollectionDetails.symbol }}</v-col
            >
          </v-row>

          <v-row>
            <v-col style="font-family: PT Sans Caption; font-weight:bold; padding:1px">
              Token Buy Price: {{ getCollectionDetails.tokenBuyPrice || 0 }} {{ getCurrency }}</v-col
            >
          </v-row>
          <v-row>
            <v-col style="font-family: PT Sans Caption; font-weight:bold; padding:1px"
              >Token Sell Price: {{ getCollectionDetails.tokenSellPrice || 0 }} {{ getCurrency }}</v-col
            >
          </v-row>
            <v-row v-if="isFundingGoalContract">
              <v-col style="font-family: PT Sans Caption; font-weight:bold; padding:1px"
                >Funding Goal: {{ getCollectionDetails.fundingGoal || 0 }} {{ getCurrency }}</v-col
              >
            </v-row>
            <v-row v-if="isFundingGoalContract">
              <v-col style="font-family: PT Sans Caption; font-weight:bold; padding:1px"
                >To reach Goal: {{ getAmountLeftForGoal || 0 }} {{ getCurrency }}</v-col
              >
            </v-row>
            <v-row v-if="!isFundingGoalContract">
              <v-col style="font-family: PT Sans Caption; font-weight:bold; padding:1px"
    ...
    <v-row>
      <v-text-field v-model="maticAmount" :rules="[numberRule]" :label="'Enter amount of ' + getCurrency"></v-text-field>
      <v-btn :disabled="isBuyingEnabled" v-on:click="buyFundTokens">
        Buy Tokens
    ...
    ><v-row>
      ...
      <v-btn :disabled="isSellingEnabled" v-on:click="sellFundTokens">
        Sell Tokens
    ...
    <v-row v-if="owner == connectedAccount">
      <v-text-field v-model="tokenBuyPrice" :rules="[numberRule]" label="Enter the new tokenPrice(In Wei) 1 ETH = 10^18 wei"></v-text-field>
      <v-btn :disabled="isSendingModify" v-on:click="setTokenPrice(true)">
        setTokenBuyPrice
    ...
    <v-row v-if="owner == connectedAccount">
      ...
      ></v-text-field>
      <v-btn :disabled="isSendingModify" v-on:click="setTokenPrice(false)">
        setTokenSellPrice
        <div v-if="loading_modify" v-cloak>
          <v-icon class="fa fa-spinner fa-spin"></v-icon>
        </div>
    ...
    <v-row v-if="owner == connectedAccount">
      <v-col>
        <v-btn :disabled="isSending_Buy" v-on:click="toggleBuyButton">
          Toggle Buy
      ...
      <v-col>
        <v-btn :disabled="isSending_Sell" v-on:click="toggleSellButton">
          Toggle Sell
        ...
        <v-dialog v-model="dialog" persistent width="500">
          ...
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn color="blue darken-1" text @click="transferFunds()">
                Continue
  ...
  computed: {
    ...
    getAmountLeftForGoal() {
      ...
      if ("contractBalance" in details && details["contractBalance"] != null) {
        diff = parseFloat(details.fundingGoal) - parseFloat(details.contractBalance);
      }
      return diff.toFixed(2);
    },
    isFundingGoalContract() {
      var details = this.$props.collections;
      console.log(details);
      if ("fundingGoal" in details) {
        return true;
      }
      return false;
    },
    ...
    transferFunds() {
      ...
      this.$store
        ...
        .then(() => {
          ...
          } else this.$vToastify.error("Some error occured");
        });
    },
  },
  data: () => ({
